...
@section('page_title')
    Laporan Sales Per Departement Kategori
@endsection
...
@section('content')
    <table class="table">
        ...
        <tr>
            <th class="left tengah">Kategori</th>
        ...
        @for($i=0; $i<sizeof($data) ;$i++)
            @if($tempDep != $data[$i]->kodedepartement )
                <tr>
                    <td colspan="9" class="left"><b>{{ $data[$i]->kodedepartement }}-{{ $data[$i]->namadepartement }}</b></td>
                </tr>
            @endif
            <tr>
                <td class="left">{{ $data[$i]->kodekategori }}-{{ $data[$i]->namakategori }}</td>
            ...
            @php
                $tempDep = $data[$i]->kodedepartement;
            ...
            @endphp
            @if( !isset($data[$i+1]->kodedepartement)  || (isset($data[$i+1]->kodedepartement) && $tempDep != $data[$i+1]->kodedepartement) )
                @php
                    if($subtotal_sales!=0){
