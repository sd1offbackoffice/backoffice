...
@section('title','OMI | LAPORAN REKAP REGISTER PPR')
@section('content')
        ...
        <div class="row">
            <div class="offset-1 col-sm-10">
                ...
                <fieldset class="card border-secondary">
                    <legend class="w-auto ml-3">REKAPITULASI REGISTER PPR</legend>
                    <div class="card-body ">
                        ...
                        <div class="row form-group">
                            <label class="col-sm-3 pl-0 pr-0 text-right col-form-label">Kode Member</label>
                            <div class="col-sm-3 buttonInside">
                                <input type="text" class="form-control text-uppercase" id="member-1">
                                <button type="button" class="btn btn-lov p-0" data-toggle="modal" data-target="#m_member"
                                        onclick="selectObjectMember(1)">
                                    <img src="{{ (asset('image/icon/help.png')) }}" width="30px">
                                </button>
                            </div>
                            <label class="col-sm-1 pt-1 text-center">s/d</label>
                            <div class="col-sm-3 buttonInside">
                                <input type="text" class="form-control text-uppercase" id="member-2">
                                <button type="button" class="btn btn-lov p-0" data-toggle="modal" data-target="#m_member"
                                        onclick="selectObjectMember(2)">
                                    <img src="{{ (asset('image/icon/help.png')) }}" width="30px">
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-sm-3 buttonInside">
                                <input type="text" class="form-control text-uppercase" id="nodoc-1">
                            ...
                            <label class="col-sm-1 pt-1 text-center">s/d</label>
                            <div class="col-sm-3 buttonInside">
                                <input type="text" class="form-control text-uppercase" id="nodoc-2">
                                <button type="button" class="btn btn-lov p-0" data-toggle="modal" data-target="#m_nodoc"
    ...
    <div class="modal fade" id="m_member" tabindex="-1" role="dialog" aria-labelledby="m_member" aria-hidden="true">
    ...
    <script>
        ...
        function getLovMember(value) {
            if($.fn.DataTable.isDataTable('#tableModalMember')){
                $('#tableModalMember').DataTable().destroy();
            }
            let tableModal = $('#tableModalMember').DataTable({
                "ajax": {
                    'url': '{{ url()->current() }}/lov-member',
                    "data": {
                        'value': value,
                ...
                "createdRow": function (row, data, dataIndex) {
                    $(row).addClass('modalRow rowMember');
                },
                "order": [],
                columnDefs: []
            });
        ...
        function getLovNodoc(value) {
            ...
            let tableModal = $('#tableModalNodoc').DataTable({
                ...
                "responsive": true,
                "createdRow": function (row, data, dataIndex) {
                    $(row).addClass('modalRow rowNodoc');
                },
                "order": [],
                columnDefs: []
            });
        ...
        function selectObjectMember(val) {
            this.idmember = val;
            getLovMember('');
        }
        ...
        $(document).on('click', '.rowMember', function () {
            var currentButton = $(this);
            let member = currentButton.children().first().text();

            $('#member-' + idmember).val(member);
            $('#m_member').modal('hide');
        });

        $(document).on('click', '.rowNodoc', function () {
            var currentButton = $(this);
        ...
        function cetak() {
            ...
            window.open(`{{ url()->current() }}/cetak?tgl1=${$('#periode-1').val()}&tgl2=${$('#periode-2').val()}&member1=${$('#member-1').val()}&member2=${$('#member-2').val()}&nodoc1=${$('#nodoc-1').val()}&nodoc2=${$('#nodoc-2').val()}&tipe=${$('input[name="tipe"]:checked').val()}`, '_blank');
        }
    </script>
