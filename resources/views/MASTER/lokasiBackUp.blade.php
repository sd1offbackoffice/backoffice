    ...
    <script>
        ...
        $(document).ready(function (){
            ...
            $('#table_lov_plu').DataTable({
                ...
                "createdRow": function (row, data, dataIndex) {
                    $(row).addClass('modalRow');
                    $(row).addClass('modalRowPlu'); //beri kelas yang berbeda
                },
                "order": [],
                columnDefs : [
                ]
        ...
        $(document).on('click', '.modalRowPlu', function () { //agar bisa pakai onclick pada modal yang berbeda
            let plu = $(this).find('td')[0]['innerHTML'];

            lov_plu_select(plu)
        } );

        //fungsi load datatables versi 2 nomor Rak
        function loadNmr(value){
            let tableNmr = $('#table_lov_rak').DataTable({
                "ajax": {
                    'url' : '{{ url()->current().'/getlokasi2' }}',
                    "data" : {
                        'value' : value
                    },
                },
        ...
        $(document).on('click', '.modalRowRak', function () {
            ...
            //let kode = currentButton.children().first().next().next().text();
            //alert(kode);
            lov_rak_select(currentButton);
        ...
        function lov_rak_select(row){
            if(row == 'input' && ($('#lks_koderak').val() == '' || $('#lks_kodesubrak').val() == '' || $('#lks_tiperak').val() == '' || $('#lks_shelvingrak').val() == '')){
                ...
                if(row != 'input'){
                    ...
                    //ambil data dari datatables rak versi 2
                ...
                if(isDPD == 'true') {
                    $('#dpd_koderak').val(data['lks_koderak']);
                    $('#dpd_kodesubrak').val(data['lks_kodesubrak']);
                    $('#dpd_tiperak').val(data['lks_tiperak']);
                    $('#dpd_shelvingrak').val(data['lks_shelvingrak']);
