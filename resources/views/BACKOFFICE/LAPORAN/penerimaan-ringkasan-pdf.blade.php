...
<html>
<head>
    <title>Daftar Penerimaan Antar Cabang Ringkasan Divisi / Departement / Kategori</title>
</head>
<body>
...
<header>
    ...
    <h2 style="text-align: center">** DAFTAR PENERIMAAN ANTAR CABANG **<br>RINGKASAN DIVISI / DEPARTEMEN / KATEGORI</h2>
</header>
...
<main>
    <table class="table">
        ...
        @for($i=0;$i<count($data);$i++)
            ...
            @if($tempkategori != $d->mstd_kodekategoribrg)
                @php $tempkategori = $d->mstd_kodekategoribrg;
                $tempgross = 0;
                $temppot = 0;
                $tempppn = 0;
                $tempbm = 0;
                ...
                for($j=$i;$j<count($data);$j++){
                    $x = $data[$j];
                    if($tempkategori == $x->mstd_kodekategoribrg){
                        $tempgross += $x->gross;
                        $temppot += $x->pot;
                        $tempppn += $x->ppn;
                        $tempbm += $x->bm;
                ...
                <tr>
                    ...
                    <td class="right">{{ number_format($tempgross,2) }}</td>
                    <td class="right">{{ number_format($temppot,2) }}</td>
                    <td class="right">{{ number_format($tempppn,2) }}</td>
                    <td class="right">{{ number_format($tempbm,2) }}</td>
                    <td class="right">{{ number_format($tempbtl,2) }}</td>
                    <td class="right">{{ number_format($tempnilai,2) }}</td>
                </tr>
            @endif
        @endfor
        <tr>
