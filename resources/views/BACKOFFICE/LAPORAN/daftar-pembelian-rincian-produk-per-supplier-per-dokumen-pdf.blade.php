    ...
    <table class="table">
        ...
        @for($i=0;$i<count($data);$i++)
                @if($tempsup != $data[$i]->msth_kodesupplier)
                    <tr>
                        <td class="left" colspan="21"><b>SUPPLIER    : {{$data[$i]->msth_kodesupplier}} - {{$data[$i]->sup_namasupplier}} </b></td>
                    </tr>
                @endif;
            ...
            @if((isset($data[$i+1]->msth_kodesupplier) && $tempsup != $data[$i+1]->msth_kodesupplier) || !(isset($data[$i+1]->msth_kodesupplier)) )
                <tr style="border-bottom: 1px solid black;">
                    ...
                    <td class="left" colspan="8">{{ $data[$i]->msth_kodesupplier }} - {{ $data[$i]->sup_namasupplier }}</td>
