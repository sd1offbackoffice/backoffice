...
@section('content')
    ...
    <table class="table">
        ...
        @for($i=0;$i<count($data);$i++)
            @php
                $data[$i]->lcost = (($data[$i]->gross - $data[$i]->potongan + $data[$i]->bm + $data[$i]->btl) * $data[$i]->frac / ($data[$i]->ctn * $data[$i]->frac + $data[$i]->pcs + $data[$i]->bonus));
            @endphp
            ...
            @if((isset($data[$i+1]->msth_nodoc) && $tempdoc != $data[$i+1]->msth_nodoc) || !(isset($data[$i+1]->msth_nodoc)) )
                <tr style="border-bottom: 1px solid black;">
                    ...
                    <th class="right">{{ number_format($st_doc_bm ,2) }}</th>
                    <th class="right">{{ number_format($st_doc_btl,2) }}</th>
