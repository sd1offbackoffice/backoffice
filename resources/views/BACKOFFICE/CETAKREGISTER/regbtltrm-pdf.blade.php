...
<html>
<head>
    <title>Register Bukti Pembatalan Penerimaan Barang</title>
</head>
<body>
...
<header>
    ...
    <h2 style="text-align: center">** REGISTER BUKTI PEMBATALAN PENERIMAAN BARANG **<br>{{ $tgl1 }} - {{ $tgl2 }}</h2>
</header>
...
<main>
    <table class="table">
        ...
        <tr>
            ...
            <th colspan="2">---- BPB ----</th>
            <th class="tengah" rowspan="2">TOP</th>
            <th class="tengah" rowspan="2">J. TEMPO</th>
        ...
        @foreach($data as $d)
            ...
            <tr>
                ...
                <td class="right">{{ number_format(round($d->potongan), 0, '.', ',') }}</td>
                <td class="right">{{ number_format(round($d->ppn), 0, '.', ',') }}</td>
                <td class="right">{{ number_format(round($d->bm), 0, '.', ',') }}</td>
                <td class="right">{{ number_format(round($d->btl), 0, '.', ',') }}</td>
            ...
            @php
                $i++;
                $subgross += $d->gross;
                $subdiscount += $d->potongan;
                $submstd_ppnrph += $d->ppn;
                $submstd_ppnbmrph += $d->bm;
                $submstd_ppnbtlrph += $d->btl;
                $subtotal += $d->total;
            @endphp
        @endforeach
        <tr>
        ...
        <tr>
            ...
            <td class="right">{{ number_format(round($pkp->ppnbm), 0, '.', ',') }}</td>
            <td class="right">{{ number_format(round($pkp->botol), 0, '.', ',') }}</td>
