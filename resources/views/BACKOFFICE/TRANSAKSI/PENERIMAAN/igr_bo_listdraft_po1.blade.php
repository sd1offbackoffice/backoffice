    ...
    <header>
        <header>
            ...
            <div style="line-height: 0.3 !important; text-align: center ;">
                ...
                <p style="text-align: center">NO. PO : {{$datas[0]->tpoh_nopo}}    <span>SUPPPLIER : </span> {{$datas[0]->supplier}}</p>
                <p style="margin-left: 77px">TANGGAL : {{date('d-m-y', strtotime($datas[0]->tpoh_tglpo))}} </p>
            </div>
        </header>
    </header>
    ...
    <main style="margin-top: 10px">
        <table class="table table-bordered table-responsive" style="">
            ...
            <tr style="text-align: center;">
                ...
                <th rowspan="2" style="width: 40px; text-align: right">HRG.SAT</th>
                <th rowspan="2" style="width: 40px; text-align: right">GROSS</th>
                <th rowspan="2" style="width: 40px; text-align: right">DISC</th>
                <th rowspan="2" style="width: 40px; text-align: right">PPN</th>
                <th rowspan="2" style="width: 40px; text-align: right">PPN BM</th>
                <th rowspan="2" style="width: 40px; text-align: right">PPN BOTOL</th>
            </tr>
            <tr>
            ...
            <tbody style="border-bottom: 1px solid black">
                @foreach($datas as $data)
                    <tr>
                        ...
                        <td style="width: 20px; text-align: right">{{$data->qty}}</td>
                        <td style="width: 20px; text-align: right">{{$data->qtyk}}</td>
                        <td style="width: 40px; text-align: right">{{number_format($data->tpod_hrgsatuan ,2,',','.')}}</td>
                        <td style="width: 40px; text-align: right">{{number_format($data->tpod_gross ,2,',','.')}}</td>
                        <td style="width: 40px; text-align: right">{{number_format($data->tpod_rphttldisc ,2,',','.')}}</td>
                        <td style="width: 40px; text-align: right">{{number_format($data->tpod_ppn ,2,',','.')}}</td>
                        <td style="width: 40px; text-align: right">{{number_format($data->tpod_ppnbm ,2,',','.')}}</td>
                        <td style="width: 40px; text-align: right">{{number_format($data->tpod_ppnbotol ,2,',','.')}}</td>
                    </tr>
                    <tr>
