...
@section('content')
    <table class="table">
        ...
        <tr>
            ...
            <th colspan="2" width="" rowspan="2">PEMBELIAN</th>
            <th colspan="2" width="" rowspan="1">PENERIMAAN</th>
            <th colspan="2" width="" rowspan="2">LAIN-LAIN</th>
            <th colspan="2" width="" rowspan="2">PENJUALAN</th>
            <th colspan="2" width="" rowspan="2">PENGELUARAN<br>LAIN-LAIN</th>
            <th colspan="2" width="" rowspan="2">INTRANSIT</th>
            <th colspan="2" width="" rowspan="2">PENYESUAIAN</th>
            <th colspan="2" width="" rowspan="2">SALDO AKHIR</th>
        </tr>
        <tr>
            <th>RETUR</th>
            <th>PENJ</th>
        ...
        @foreach($data as $d)
            @if($plu != $d->ktg_prdcd)
                ...
                @if($plu != null)
                    <tr class="bold top-bottom">
                        ...
                        <td class="right">{{ $tot_cp_tinq1 }}</td>
                        <td class="right">{{ $tot_cp_tinf1 }}</td>
                        <td class="right">{{ $tot_cp_tinq2 }}</td>
                        <td class="right">{{ $tot_cp_tinf2 }}</td>
                        <td class="right">{{ $tot_cp_tinq3 }}</td>
                        <td class="right">{{ $tot_cp_tinf3 }}</td>
                        <td class="right">{{ $tot_cp_toutq1 }}</td>
                        <td class="right">{{ $tot_cp_toutf1 }}</td>
                        <td class="right">{{ $tot_cp_toutq3 }}</td>
                        <td class="right">{{ $tot_cp_toutf3 }}</td>
                        <td class="right">{{ $tot_cp_intrq }}</td>
                        <td class="right">{{ $tot_cp_intrf }}</td>
                        <td class="right">{{ $tot_cp_tinq4 }}</td>
                        <td class="right">{{ $tot_cp_tinf4 }}</td>
                        <td class="right">{{ $tot_cp_sakhirq }}</td>
                        <td class="right">{{ $tot_cp_sakhirf }}</td>
                    </tr>
            ...
            @php
                ...
                $p_sawal = $cp_tsaq1;
                $cp_sakhirq = round($p_sawal / $d->frac);
                $cp_sakhirf = $p_sawal % $d->frac;
                $p_sakhirq = round($p_sawal / $d->frac);
                $p_sakhirf = $p_sawal % $d->frac;

                $cp_tinq1 = round($d->asaqty / $d->frac);
                $cp_tinq2 = round($d->csaqty / $d->frac);
                $cp_tinq3 = round($d->bsaqty / $d->frac);
                $cp_tinq4 = round($d->isaqty / $d->frac);
                $cp_tinf1 = $d->asaqty % $d->frac;
                $cp_tinf2 = $d->csaqty % $d->frac;
                $cp_tinf3 = $d->bsaqty % $d->frac;
                $cp_tinf4 = $d->isaqty % $d->frac;
                $cp_toutq1 = round($d->dsaqty / $d->frac);
                $cp_toutq3 = round($d->fsaqty / $d->frac);
                $cp_toutf1 = $d->dsaqty % $d->frac;
                $cp_toutf3 = $d->fsaqty % $d->frac;
                $cp_intrq = round($d->qintr / $d->frac);
                $cp_intrf = $d->qintr % $d->frac;
                $cp_gntrq = round($d->gintr / $d->frac);
            ...
            <tr>
                ...
                <td class="right">{{ $cp_toutq1 }}</td>
                <td class="right">{{ $cp_toutf1 }}</td>
                <td class="right">{{ $cp_toutq3 }}</td>
