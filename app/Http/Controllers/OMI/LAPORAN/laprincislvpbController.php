...
ORDER BY pbo_kodeomi, pbo_pluigr");
        ...
        for($i=0;$i<sizeof($datas);$i++){
            if((int)$datas[$i]->qtyo != 0 && (int)$datas[$i]->qtyr != 0){
                $prsq[$i] = round(((int)$datas[$i]->qtyr / (int)$datas[$i]->qtyo) * 100,2);
            }else{
                $prsq[$i] = 0;
            }

            if((int)$datas[$i]->nilaio != 0 && (int)$datas[$i]->nilair != 0){
                $prsr[$i] =  round(((int)$datas[$i]->nilair / (int)$datas[$i]->nilaio) * 100,2);
            }else{
                $prsr[$i] = 0;
            }
        }
