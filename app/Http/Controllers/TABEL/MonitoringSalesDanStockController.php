        ...
        $data = DB::connection($_SESSION['connection'])
            ->select("
                                SELECT DISTINCT
                                       ...
                                       mpl_prdcd, prd_deskripsipendek, prd_unit||'/'||prd_frac kemasan,
                                       ...
                                       END,0) avgqty,
                                       NVL(st_sales,0) sales, NVL(st_saldoakhir,0) saldo, NVL(mgrA,0) margin, NVL(mgrB,0) margin2, NVL(pkm_pkmt,0) ftpkmt,
...
//        dd($data);
        ...
        for ($j = 0; $j < sizeof($data); $j++) {
            ...
            $param = 5;
            $tglVV = DateTime::createFromFormat('d/m/Y', $tglVV);
//            dd($tglVV->add( new DateInterval('P'.$param.'D'))->format('d/m/Y'));
            for ($i = 2; $i <= 7; $i++) {
                if (substr($data[$j]->hrkj, $i, 1) == 'Y') {
                    ...
                    if ($i < $dow) {
                        ...
                        if ($tempHL <> 0) {
                            $tglY = $tglL->sub(new DateInterval('P' . $ltimeA . 'D'))->sub(new DateInterval('P' . '1' . 'D'));

                        } else {
                            $tglY = $tglL->sub(new DateInterval('P' . $ltimeA . 'D'));
                        }
                    } else {
                        $tglL = DateTime::createFromFormat('d/m/Y', $lastPer)->add(new DateInterval('P' . $yT . 'D'));
                        $tglY = $tglL->sub(new DateInterval('P' . $ltimeA . 'D'));

                        if (DateTime::createFromFormat('d/m/Y', $lastPer) > $tglY) {
                            $tglL = DateTime::createFromFormat('d/m/Y', $lastPer)->add(new DateInterval('P' . '7' . 'D'))->add(new DateInterval('P' . $yT . 'D'));
                            $tglY = $tglL->sub(new DateInterval('P' . $ltimeA . 'D'));
                        }
                        ...
                        if ($tempHL <> 0) {
                            $tglY = $tglY->sub(new DateInterval('P' . '1' . 'D'));
                        }
            ...
            $tglPB = Self::nvl($tglVV, '');
            if (!isset($supp[0]->hrkj)) {
                $keterangan = '';
            }

            if ($tglPB != 'Tidak Ada' && $tglPB != '') {
                $tglPB = $tglPB->format('d/m/Y');
            }
            $data[$j]->cp_tglpb = $tglPB;
            $data[$j]->keterangan = $keterangan;
    ...
    public function getData2(Request $request)
    {
        ...
        for ($k = 0; $k < sizeof($data); $k++) {
                    ...
                    (	SELECT DISTINCT
                        ...
                        (	 SELECT tpod_prdcd, SUM(qtyB) qtyB
                             ...
                             (  SELECT DISTINCT tpod_prdcd, NVL((tpod_qtyPO),0) qtyB
                                    ...
                                    WHERE tpoh_nopo = tpod_nopo
                                        AND tpod_prdcd = SUBSTR('" . $data[$k]->mpl_prdcd . "',1,6)||'0'
                                        AND tpod_qtyPB IS NULL AND tpod_recordid IS NULL
                                        AND TRUNC(SYSDATE) BETWEEN tpoh_tglPO AND (tpoh_tglPO + tpoh_jwpb)
                             )
                             GROUP BY tpod_prdcd
                        ...
                        (	 SELECT pbd_prdcd, SUM(qtyC) qtyC
                             ...
                             (	SELECT DISTINCT pbd_prdcd, NVL((pbd_qtyPB),0) qtyC
                                    ...
                                    WHERE pbh_nopb = pbd_nopb
                                        AND pbd_prdcd = SUBSTR('" . $data[$k]->mpl_prdcd . "',1,6)||'0'
                                        AND pbd_recordid IS NULL AND pbd_qtybpb IS NULL
                                        AND TRUNC(SYSDATE) BETWEEN pbh_tglPB AND (pbh_tglPB+2)
                             )
                             GROUP BY pbd_prdcd
                        ...
                        WHERE mpl_kodeigr = '" . $_SESSION['kdigr'] . "'
                            ...
                            AND sls_prdcd = '" . $data[$k]->mpl_prdcd . "'
                            AND prd_prdcd = '" . $data[$k]->mpl_prdcd . "'
                    )
            ...
            $data[$k]->sales = $proses[0]->sales_;
            $data[$k]->margin = $proses[0]->margin;
            $data[$k]->margin2 = $proses[0]->margin2;
            $data[$k]->saldo = $proses[0]->saldo;
            $data[$k]->deskripsi = $proses[0]->deskripsi;
            ...
            if (sizeof($supp) > 0) {
                if ($supp[0]->jwpb == 0) {
                    $ltimeA = 4;
                } else if ($supp[0]->jwpb > 5) {
                    $ltimeA = 6;
                } else {
                    $ltimeA = $supp[0]->jwpb + 1;
                }
            ...
            $tglPB = Self::nvl($tglPB, '');
            if ($tglPB != '' && $tglPB != '') {
                $tglPB = $tglPB->format('d/m/Y');
            }
            $data[$k]->tglpb = Self::nvl($tglPB, '');
...
//            dd($proses);
    ...
    public function print(Request $request)
    {
        ...
        for ($j = 0; $j < sizeof($data); $j++) {
            ...
            $param = 5;
            $tglVV = DateTime::createFromFormat('d/m/Y', '13/12/9999');
//            dd($tglVV->add( new DateInterval('P'.$param.'D'))->format('d/m/Y'));
            for ($i = 2; $i <= 7; $i++) {
                if (substr($data[$j]->hrkj, $i, 1) == 'Y') {
                    $yT = abs($dow - $i);
        ...
        return view('TABEL.' . $filename, compact(['perusahaan', 'data', 'periode', 'namabulan', 'kodemonitoring']));
    }
