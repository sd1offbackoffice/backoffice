...
class PLUNonPromoController extends Controller
{
    ...
    public function getData(){
        $data = DB::connection(Session::get('connection'))->table('tbmaster_plunonpromo')
    ...
    public function addPLU(Request $request){
        $temp = DB::connection(Session::get('connection'))->table('tbmaster_plunonpromo')
            ->where('non_kodeigr','=',Session::get('kdigr'))
            ->where('non_prdcd','=',$request->plu)
            ->first();

        if(!$temp){
            DB::connection(Session::get('connection'))->table('tbmaster_plunonpromo')
                ->insert([
