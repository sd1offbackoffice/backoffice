...
class LaporanSalesPerDivDepController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        $dataDivGroup = DB::connection(Session::get('connection'))
            ->select("select kodedivisi,namadivisi nama,case when sum(sales)=0 then 0 else round(sum(margin)/sum(sales)*100,2) end  marginpersen ,sum(qty) qty,sum(sales) sales,sum(margin) margin,sum(jmlmember) jmlmember from (
                                select a.*
                                from(
