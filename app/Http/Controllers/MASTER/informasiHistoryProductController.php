...
class informasiHistoryProductController extends Controller
{
    ...
    {
//        $subquery1 = DB::Raw("SELECT DISTINCT kki_periode FROM tbupload_kkei)");
//        $subquery1 = DB::connection($_SESSION['connection'])->table('tbupload_kkei')
//            ->select('kki_periode')->distinct()->toSql();
//
//        $subquery1 = DB::Raw('('.$subquery1.')');
//
////        $subquery2 = DB::Raw("(SELECT DISTINCT pbd_nopb,
////                                    pbh_tglpb,
////                                    pbd_nopo,
////                                    tpoh_tglpo
...
////                           AND tpoh_nopo(+) = pbd_nopo) C");
////
////        $subquery3 = DB::Raw("(SELECT msth_nodoc, msth_tgldoc, msth_nopo
////                      FROM tbtr_mstran_h@igrgij
////                     WHERE msth_typetrn = 'B') D");
//
//        $query = $so = DB::connection($_SESSION['connection'])->table(DB::Raw('tbdownload_kkei@igrgij'))
//            ->join($subquery1, function ($join) {
//                $join->On('kke_periode', '=', 'kki_periode');
//            })
//            ->leftJoin($subquery2, function ($join) {
//                $join->On('pbd_nopb', '=', 'kke_nomorpb');
//            })
//            ->leftJoin($subquery3, function ($join) {
//                $join->On('msth_nopo', '=', 'kki_periode');
//            })
//            ->select('*')
//            ->where('kke_cabang', '=', '22')
//            ->whereRaw(' TO_CHAR (kki_periode, \'yyyyMM\') >=
...
//	        NEXT_RECORD;
        ...
        for ($i = 0; $i < sizeof($hargabeli); $i++) {
            ...
            if (Self::ceknull($hargabeli[$i]->fmbntm, "") != "") {
                $hb_periodbns = $hargabeli[$i]->fmbntm . ' s/d ' . $hargabeli[$i]->fmbnta;
            }
