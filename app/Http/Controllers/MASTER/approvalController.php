...
class approvalController extends Controller
{
    public function index(){
        ...
        return view('MASTER.approval')->with('result',$result);
    }
    ...
    public function saveData(Request $request){
        ...
        if ($result==0) {
            DB::table('TBMASTER_REPORT_APPROVAL')->insert(
                ['RAP_KODEIGR' => '22', 'RAP_STORE_MANAGER' => $request->storemanager, 'RAP_STORE_ADM' => $request->storeadm, 'RAP_LOGISTIC_SUPERVISOR' => $request->logisticsupervisor, 'RAP_STOCKKEEPER_II' => $request->stockkeeper, 'RAP_ADMINISTRASI' => $request->administrasi, 'RAP_KEPALAGUDANG' => $request->kepalagudang]
            );
            return 'save';
        }
        else{
