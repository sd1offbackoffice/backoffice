...
class pembatalanBAPemusnahanController extends Controller
{
    ...
    public function getDocument(Request $request){
        ...
        $datas = DB::connection(Session::get('connection'))->table('tbtr_mstran_d')
    ...
    public function getDetailDocument(Request $request){
        ...
        $detail = DB::connection(Session::get('connection'))->table('tbtr_mstran_d')
...
//        ------------- Cek apakah sudah closing atau belum
        $tgltran = DB::connection(Session::get('connection'))->select("SELECT to_char(msth_tgldoc,'YYYYMM') as data
                    FROM tbtr_mstran_h
        ...
        $periode = DB::connection(Session::get('connection'))->select("SELECT prs_tahunberjalan || prs_bulanberjalan as data FROM tbmaster_perusahaan");
...
//        ------------- Cek apakah akan dilakukan lopping atau tidak
        ...
        $datas   = DB::connection(Session::get('connection'))->select("SELECT mstd_prdcd, mstd_nodoc, mstd_tgldoc,
        ...
        foreach ($datas as $data){
            $temp   = DB::connection(Session::get('connection'))->select("SELECT NVL (COUNT (1), 0) as nvl
                                         FROM tbmaster_stock
            ...
            if ($temp[0]->nvl == 0){
                DB::connection(Session::get('connection'))->table('tbmaster_stock')->insert(['st_prdcd' => $data->mstd_prdcd, 'st_lokasi' => '03']);
            }
            ...
            $temp2  = DB::connection(Session::get('connection'))->select("SELECT st_avgcost, st_saldoakhir, st_trfout
                                         FROM tbmaster_stock
