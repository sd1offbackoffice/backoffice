...
//            Update tbtr_barangrusak (rsk_Record_id = '2')
            DB::table('tbtr_barangrusak')->where('rsk_nodoc', $noPBBR)->update(['rsk_recordid' => '2', 'rsk_modify_by' => $userid, 'rsk_modify_dt' => $today]);

            return response()->json(['kode' => 1, 'msg' => $newNoDoc]);
        } else {
//            *** Update Data ***
            DB::table('tbtr_bpb_barangrusak')->where('brsk_nodoc', $noDoc)->delete();
...
//        Get No Ref/PBBR
        $getNoPBBR  = DB::table('tbtr_bpb_barangrusak')->select('brsk_tgldoc', 'brsk_noref', 'brsk_tglref')->where('brsk_nodoc', $noDoc)->first();

//        Cek Status New or Update
        $getDoc = DB::table('tbtr_mstran_h')->where('msth_nodoc', $noDoc)->where('msth_noref3', $getNoPBBR->brsk_noref)->get()->toArray();

        if(!$getDoc){
//            Insert into tbtr_mstran_h
            DB::table('tbtr_mstran_h')->insert(['msth_kodeigr' => $kodeigr, 'msth_typetrn' => 'F', 'msth_nodoc' => $noDoc, 'msth_tgldoc' => $getNoPBBR->brsk_tgldoc, 'msth_noref3' => $getNoPBBR->brsk_noref, 'msth_tgref3' => $getNoPBBR->brsk_tglref,
                'msth_flagdoc' => '1', 'msth_create_by' => $userid, 'msth_create_dt' => $today, 'msth_modify_by' => $userid, 'msth_modify_dt' => $today]);

//            Get prdcd to looping
            $prdcd  = DB::table('tbtr_bpb_barangrusak')->select('*')->where('brsk_nodoc', $noDoc)->get()->toArray();
...
//                Get Data to REC
                $rec    = DB::select(" SELECT prd_kodesupplier, prd_kodedivisi,
...
//                Insert into tbtr_mstran_d
                ...
                DB::table('tbtr_mstran_d')->insert(['mstd_kodeigr' => $kodeigr, 'mstd_typetrn' => 'F', 'mstd_nodoc' => $noDoc, 'mstd_tgldoc' => $getNoPBBR->brsk_tgldoc,
