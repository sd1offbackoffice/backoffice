    ...
    public function getData(Request $request)
    {
        ...
        if ($jenis == 1) {
            $data = DB::connection(Session::get('connection'))->table('tbtr_backoffice')
                ...
                ->distinct()
                ->get();
        } else if ($jenis == 2) { //query masih sama sama list 
            $data = DB::connection(Session::get('connection'))->table('tbtr_backoffice')
                ->selectRaw("trbo_nodoc no, trbo_tgldoc tgl")
                ->where('trbo_typetrn', '=', 'O')
                ->whereNull('trbo_recordid')
            ...
            if ($jenis == 1) {
                    ...
                    "SELECT trbo_nodoc,
                            ...
                            CASE
                                ...
                                WHEN trbo_flagdisc1 = '3'
                                    ...
                                    END
                                    END
                                    END AS ket, prd_deskripsipanjang, prd_unit, prd_frac, trbo_loc, cab_namacabang, trbo_gdg, gdg_namagudang
                ...
                if (!$reprint) {
                    foreach ($nodoc as $knodoc) {
                        ...
                        $s = oci_parse($c, "BEGIN :ret := F_IGR_GET_NOMOR('$kodeigr','SJK','Nomor Surat Jalan','3' || TO_CHAR(SYSDATE, 'yy'),5,TRUE); END;");
                        ...
                        foreach ($data as $d) {
                            ...
                            $c = loginController::getConnectionProcedure();
                            $s = oci_parse($c, "BEGIN SP_IGR_UPDATE_STOCK_MIGRASI('$kodeigr','01','" . $d->trbo_prdcd . "','" . $d->trbo_kodesupplier . "','TRFOUT','" . $d->trbo_qty . "','" . $d->st_lastcost . "','" . $d->st_avgcost . "','" . Session::get('usid') . "',:vlok,:vmessage); END;");
                ...
                $data = DB::connection(Session::get('connection'))->select("SELECT msth_recordid, msth_nodoc, msth_tgldoc, msth_nopo, msth_tglpo,
                        ...
                        mstd_flagdisc1,msth_loc,cab_namacabang, cab_alamat1, cab_alamat2, cab_alamat3, msth_loc2,gdg_namagudang,
                ...
                $pdf = PDF::loadview('BACKOFFICE.TRANSAKSI.KIRIMCABANG.surat_jalan_expedisi', compact(['perusahaan', 'data', 'reprint']));
                $pdf->output();
                ...
                return $pdf->stream('surat_jalan_expedisi.PDF');
            } else {
                $reprint = DB::connection(Session::get('connection'))
                    ->table('tbtr_mstran_h')
                    ->whereIn('msth_nodoc', $nodoc)
