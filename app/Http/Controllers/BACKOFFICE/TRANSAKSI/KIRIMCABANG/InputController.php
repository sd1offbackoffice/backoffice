...
ORDER BY PRDCD,
    ...
    public function saveDataTrn(Request $request)
    {
        ...
        // TITIP
        $nosj = $request->nosj ? $request->nosj : '';
        $tgleta = $request->tgleta ? $request->tgleta : Carbon::now();
        $tgletd = $request->tgletd ? $request->tgletd : Carbon::now();
        $kodecabangtitip = $request->kodecabangtitip ? $request->kodecabangtitip : '';
        $namacabangtitip = $request->namacabangtitip ? $request->namacabangtitip : '';
        $ekspedisi = $request->ekspedisi ? $request->ekspedisi : '';
        $seal = $request->seal ? $request->seal : '';
        $nomobil = $request->nomobil ? $request->nomobil : '';
        $durasi = $request->durasi ? $request->durasi : '';
        $tarif = $request->tarif ? $request->tarif : '';
        $alamat = $request->alamat ? $request->alamat : '';
        $container = $request->container ? $request->container : '';
        $kapal = $request->kapal ? $request->kapal : '';
        $titip_nama_barang = $request->titip_nama_barang ? $request->titip_nama_barang : '';
        $titip_kode = $request->titip_kode ? $request->titip_kode : 'EKSPEDISI';
        $titip_frac = $request->titip_frac ? $request->titip_frac : '';
        $titip_qty = $request->titip_qty ? $request->titip_qty : '';
        $titip_m3 = $request->titip_m3 ? $request->titip_m3 : '';
        $titip_ton = $request->titip_ton ? $request->titip_ton : '';
        $titip_keterangan_titipan = $request->titip_keterangan_titipan ? $request->titip_keterangan_titipan : '';
        $koli = $request->koli ? $request->koli : '';
        $kodeeks = $request->kodeeks ? $request->kodeeks : '';
        try {
            DB::connection(Session::get('connection'))->beginTransaction();
