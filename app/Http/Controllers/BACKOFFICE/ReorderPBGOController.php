...
class ReorderPBGOController extends Controller
{
    ...
    public function proses_goOld(){
        ...
        else $NILAIPPN = ($prs->prs_nilaippn/100);
...
//            ->get();
        ...
        try{
            ...
            foreach($pb as $b){
                ...
                if($data['prdcd'] == $b->tpod_prdcd){
                    ...
                    $data['recid'] = $TEMP_RECID;
                    $data['docno'] = $NOPB;
                    $data['tgl'] = DB::connection(Session::get('connection'))->RAW("to_date('".$TGLAKHIR."','YYYY-MM-DD  HH24:MI:SS')");
        ...
        $insert_tbtr_pb_h['pbh_tglpb'] = DB::connection(Session::get('connection'))->RAW("to_date('".$TGLAKHIR."','YYYY-MM-DD  HH24:MI:SS')");
...
//            DB::connection(Session::get('connection'))->commit();
    ...
    public function proses_go(Request $request){
        set_time_limit(0);

        $count = $pb = DB::connection(Session::get('connection'))->table('tbtr_po_d')
            ->join('tbmaster_prodmast','tpod_prdcd','prd_prdcd')
            ->join('tbtr_po_h','tpod_nopo','tpoh_nopo')
            ->join('tbtr_mstran_h','tpoh_nopo','msth_nopo')
            ->leftJoin('tbmaster_tag','prd_kodetag','tag_kodetag')
...
//            ->get();
        ...
        if($request->final == 'true'){
            self::process2($request,$TGLAKHIR);
        }
    }

    public function process2(Request $request, $TGLAKHIR){
        $pbprint = DB::connection(Session::get('connection'))->table('temp_pbprint')
            ->where('recid',null)
            ->whereRaw(DB::connection(Session::get('connection'))->raw("TRUNC(tgl) = TRUNC(to_date('".$TGLAKHIR."','YYYY-MM-DD  HH24:MI:SS'))"))
            ->where('docno',$request->nopb)
            ->orderBy('supco')
            ->get();
...
//            DB::connection(Session::get('connection'))->commit();
    ...
    public function cetak_tolakan(Request $request){
        $recid = $request->recid;
        $nopb = $request->nopb;
