...
class maxpaletUntukPBController extends Controller
{
    ...
    public function loadData(){
        $datas  = DB::connection(Session::get('connection'))->table('tbmaster_pb_maxpalet a')
    ...
    public function getMaxPalet(Request $request){
        ...
        $plu    = DB::connection(Session::get('connection'))->select("SELECT LPAD ( RPAD (SUBSTR ('$kodePlu', 0, LENGTH ('$kodePlu') - 1),LENGTH ('$kodePlu'),'0'), 7, '0') AS plu FROM DUAL");
        $plu    = $plu[0]->plu;
        ...
        $searchMaxPalet = DB::connection(Session::get('connection'))->table('tbmaster_maxpalet')->where('mpt_prdcd', $plu)->get()->toArray();
    ...
    public function saveData(Request $request){
        ...
        $search     = DB::connection(Session::get('connection'))->table('tbmaster_prodmast')->where('prd_prdcd', $kodePlu)->get()->toArray();

        $search2    = DB::connection(Session::get('connection'))->table('tbmaster_pb_maxpalet')->where('pmp_prdcd', $kodePlu)
